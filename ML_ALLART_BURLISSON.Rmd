...
Les données utilisées dans ce projet proviennent de Kaggle, où elles ont été initialement proposées dans le cadre d’une compétition intitulée **Health Insurance Cross Sell Prediction** (https://www.kaggle.com/datasets/anmolkumar/health-insurance-cross-sell-prediction). La compétition avait pour objectif de prédire si un client existant pourrait être intéressé par une assurance véhicule, en utilisant les informations démographiques, les caractéristiques des véhicules et les détails de la police d’assurance.
...
L’objectif principal est de construire un modèle prédictif en utilisant des données démographiques, des informations sur les véhicules, et des données de politiques d’assurance. Ce modèle doit répondre à la question : **Un client actuel serait-il intéressé par une assurance véhicule ?**
...
```
![Importance des  ](figures/my_plot_1-1.png)
...
Ces approches peuvent aider à améliorer la performance des modèles en réduisant l'impact du déséquilibre des classes sur les prédictions.

*Approche proposée*

Pour notre analyse, nous examinerons l'impact de ces techniques sur les performances du modèle. En particulier, nous évaluerons si le **suréchantillonnage** ou le **sous-échantillonnage** permet d'augmenter la précision globale ou la sensibilité aux prédictions de la classe minoritaire.
...
```
![Importance](figures/my_plot_3-1.png)
...
Pour cette analyse, les âges ont été regroupés en classes comme suit :

| **Âge** | **Tranche d'âge (AgeClass)** |
|---------|-------------------------------|
| 44      | 40-49                        |
| 76      | 70-79                        |
| 47      | 40-49                        |
...
```
![Importance](figures/my_plot_4-1.png)
...
![Importance](figures/my_plot_5-1.png)

## Detection de valeurs aberrantes et manquantes
...
**Action entreprise** : Pour garantir la robustesse des analyses et des modèles, nous avons décidé de **supprimer les lignes contenant des valeurs aberrantes** dans la variable `Annual_Premium`. Cette approche permet de travailler avec des données plus homogènes tout en réduisant l'impact des valeurs extrêmes sur les résultats.


## Division en ensembles d'entraînement et de test et encodage des variables catégoriques

### 1. Division en ensembles d'entraînement et de test
...
```{r my_plot_6, fig.path='figures/', dev='png', echo=FALSE, include=FALSE, results='hide', message=FALSE, warning=FALSE}
...
![Importance](figures/my_plot_6-1.png)

## Suréchantillonage avec ROSE
...
L’analyse en composantes principales (ACP) est une méthode de réduction dimensionnelle qui permet de transformer les variables d’un jeu de données en un ensemble de composantes principales. Ces composantes principales expliquent la variance maximale des données tout en réduisant la complexité du modèle.


*Objectif de l'ACP*

L'objectif principal de cette ACP est de :
- Visualiser les données dans un espace réduit à deux dimensions pour mieux comprendre leur distribution.
- Identifier si les classes (`Response` = 0 et 1) sont bien séparées dans cet espace.
- Évaluer la variance totale expliquée par les deux premières composantes principales.


*Résultats*

- Les deux premières composantes principales expliquent **40.58 % de la variance totale** des données.
- Le graphique ci-dessous montre une séparation notable entre les classes `0` (en bleu) et `1` (en rouge) dans l’espace des deux premières composantes principales. Cela indique que les données contiennent des informations discriminantes qui peuvent être utiles pour la classification.


```{r my_plot_7, fig.path='figures/', dev='png', echo=FALSE, include=FALSE, results='hide', message=FALSE, warning=FALSE}
# Installer et charger les bibliothèques nécessaires

# Effectuer une ACP sur les données suréchantillonnées
pca <- prcomp(all_train_enc_over_sampled, scale. = TRUE)

# Obtenir les composantes principales
components <- as.data.frame(pca$x[, 1:2])  # Sélectionner les 2 premières composantes principales
components$Response <- as.factor(y_train)  # Ajouter la variable cible pour la couleur

# Calculer la variance totale expliquée
explained_variance <- sum(pca$sdev[1:2]^2) / sum(pca$sdev^2) * 100

# Créer un graphique de dispersion
ggplot(components, aes(x = PC1, y = PC2, color = Response)) +
  geom_point(alpha = 0.7, size = 2) +
  scale_color_manual(values = c("blue", "red")) +
  labs(title = sprintf("Total Explained Variance: %.2f%%", explained_variance),
       x = "Principal Component 1",
       y = "Principal Component 2") +
  theme_minimal()

```
![Importance](figures/my_plot_7-1.png)
...
# Entraîner le normaliseur (StandardScaler en Python)
...
Tous Tous les modèles sont entraînés en utilisant la fonction `caret::train()`. Et on utilise la méthode de validation croisée (5-fold) pour évaluer les performances des modèles avec comme métrique principale : **ROC AUC**. 
...
ggplot(cv_results_long, aes(x = Model, y = ROC, fill = Model)) +
  geom_boxplot() +
  labs(title = "Comparison of ROC Results Across Models",
       x = "Models",
       y = "ROC Score") +
  theme_minimal() +
...
```
![Importance](figures/my_plot_8-1.png)
...
```{r my_plot_9_10, fig.path='figures/', dev='png', echo=FALSE, include=FALSE, results='hide', message=FALSE, warning=FALSE}
library(gridExtra)
grid.arrange(g1, g2, ncol = 2)
```

![Importance](figures/my_plot_9_10-1.png)
...
```{r my_plot_11, fig.path='figures/', dev='png', echo=FALSE, echo=FALSE, include=FALSE, results='hide', message=FALSE, warning=FALSE }
library(caret)
library(xgboost)
...
```
![Importance](figures/my_plot_11-1.png)
...
Les valeurs SHAP jouent un rôle clé dans l'interprétabilité des modèles complexes et permettent de valider les décisions prises par le modèle.
...
2. **Interprétation des caractéristiques** :
   ...
   - Des caractéristiques moins influentes, comme **`Gender`** ou **`AgeClass`**, pourraient être exclues dans de futures itérations.
...
2. **Perspectives** :
   ...
   - Tester d'autres algorithmes avancés, comme les réseaux neuronaux, pour des comparaisons supplémentaires.
